...
def forward_video(rgbs, framerate, model, args):
    ...
    rgb_out_f = os.path.join(args.save_path, 'pt_vis_%s_rate%d_q%d.mp4' % (fn, rate, args.query_frame))
    print('rgb_out_f', rgb_out_f)
    temp_dir = os.path.join(args.save_path, 'temp_pt_vis_%s_rate%d_q%d' % (fn, rate, args.query_frame))
    ...
    # 保存为NumPy格式而不是JSON，更快更小
    save_path = args.save_path
    save_path = os.path.join(save_path, 'pt_vis_%s_rate%d_q%d.npz' % (fn, rate, args.query_frame))
    print('save_path', save_path)
    
    # 保存为压缩的NumPy格式
    np.savez_compressed(save_path, 
                       trajs=trajs_e.cpu().numpy(),
                       visconfs=visconfs_e.cpu().numpy())
    
    # json_data = {}
    # json_data['trajs'] = trajs_e.cpu().numpy().tolist()
    # json_data['visconfs'] = visconfs_e.cpu().numpy().tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default='./pt_vis')
